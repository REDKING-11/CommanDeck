...
public class ActionButtonData {
    ...
    public void run(boolean isKeybind) {
        ...
        if (displayRunText == ModConfig.DisplayRunText.ALWAYS || (displayRunText == ModConfig.DisplayRunText.KEYBIND_ONLY && isKeybind)) {
            if (Minecraft.getInstance().player != null) {
                Minecraft.getInstance().player.sendOverlayMessage(Component.literal("Ran action \"" + name + "\""));
            }
        }
    ...
    public static class CustomModelDataValues {
        ...
        public CustomModelData getComponent() {
            return new CustomModelData(floatList, List.of(), stringList, List.of());
        }
    }
}
